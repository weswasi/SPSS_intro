...
SPSS introduktion ger en guide till att arbeta med \textbf{SPSS Version 28} vilket är den senaste versionen och som vi rekommenderar att ni använder. Tidigare versioner av SPSS kan avvika något gälland tillvägagångsätten för vissa av de instruktioner som här presenteras.
...
variabelvärden klickar du \textbf{OK}.
...
varians reflekterar en skillnad i populationernas varians. Om så är fallet - vilket det emellertid inte är här (P = 0,10) - går vi till \textbf{Equal variance assumed} (den översta raden). Om Sig.-värdet understiger .05 så betyder det att \textbf{Equal variance not assumed} (variansen inte den samma mellan grupperna) och vi använder oss av t-test på den undre raden. ''Equal variance assumed'' är mer restriktivt vilket betyder att det är svårare att få ett signifikant resultat.
