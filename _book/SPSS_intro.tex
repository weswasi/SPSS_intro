...
varians reflekterar en skillnad i populationernas varians. Om så är fallet - vilket detär här eftersom (P = 0,01) - går vi till \textbf{Equal variance assumed} (den översta raden). Om Sig.-värdet understiger .05 så betyder det att \textbf{Equal variance not assumed} (variansen inte den samma mellan grupperna) och vi använder oss av t-test på den undre raden. ''Equal variance assumed'' är mer restriktivt vilket betyder att det är svårare att få ett signifikant resultat.
